...
{
    Vertex vert = { 0 };
    u32 size = region_array_size(vert_array);
    for (u32 i = 0; i < poly.n_sides; i++)
    {
        vert.color = color;
        vert.pos = v3f(poly.points[i].x, poly.points[i].y, z);
...
{
    region_stack_begin_scope(corner_stack);
    ...
    V2* vert_pos = region_stack_malloc(corner_vertices_count * 4, V2);
    ...
    region_stack_end_scope(corner_stack);
}
