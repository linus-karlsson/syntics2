...
{
    region_stack_begin_scope(f_parse_stack);
    u32 i0 = 0, i1 = 0, i2 = 0;

    char* current_pos = line;

    Indices* indices_array = region_stack_array(100, Indices);
    ...
    const u32 size = region_array_size(indices_array);
    ...
    region_stack_end_scope(f_parse_stack);
}
...
{
    region_stack_begin_scope(model_load_stack);
    File_Attrib file = { 0 };
    platform_file_read(&file, region_stack_get(), model_path);

    _buffer_parse(obj_attrib, &file);
    region_stack_end_scope(model_load_stack);
}
