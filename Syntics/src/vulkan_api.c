...
void vulkan_init(Region_Alloc* region, Instance_State* instance_state, Application_State* app_state,
                 Render_State** render_state, u32 width, u32 height)
{
    assert(!VULKAN_API_INITIALIZED);
    if (vulkan_enable_validation()) vulkan_debug_messenger_init(instance_state);

    vulkan_surface_create(app_state->platform, instance_state->instance, &app_state->surface);

    vulkan_pick_physical_device(instance_state->instance, app_state->surface,
                                &app_state->phy_device, &app_state->q_indices);

    vulkan_logical_device_create(app_state->phy_device, app_state->q_indices, &app_state->device);

    Queues queue = { 0 };
    vkGetDeviceQueue(app_state->device, app_state->q_indices.indices[GRAPHICS_QUEUE_IDX], 0,
                     &queue.graphic_queue);

    vkGetDeviceQueue(app_state->device, app_state->q_indices.indices[GRAPHICS_QUEUE_IDX], 0,
                     &queue.present_queue);

    vulkan_command_pool_create(app_state->device, app_state->q_indices.indices[GRAPHICS_QUEUE_IDX],
                               &app_state->com_pool);
    ...
    vulkan_swapchain_create(app_state->phy_device, app_state->device, app_state->surface, width,
                            height, app_state->q_indices, VK_NULL_HANDLE, false,
    ...
    vulkan_swapchain_get_images(region, app_state->device, &app_state->swap_chain);
    ...
    vulkan_render_state_init(region, app_state->device, queue, app_state->phy_device,
