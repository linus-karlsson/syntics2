...
{
    Recreate_Callback rc_callback;
    void* data;
} Recreate_Task;

typedef struct Recreate_Graphic_Pipeline_Task
{
    Recreate_Graphic_Pipeline_callback rc_gp_callback;
    void* data;
...
thread_return_value looking_for_file_changes(void* data)
{
    ...
    for (;;)
    {
        platform_semaphore_wait_and_decrement(&state->start_semaphore);
...
void vulkan_fence_and_semaphore_create(VkDevice device, VkFence* fence,
                                       VkSemaphore* image_semaphores,
                                       VkSemaphore* present_semaphores)
{
                              ...
                              Render_State** render_state)
{
    ...
    platform_thread_create(state_internal, looking_for_file_changes, 0, NULL);
