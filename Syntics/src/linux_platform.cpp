...
void init_platform(const char* title, uint16 width, uint16 height)
{
    ...
    xcb_internal_contex.display = XOpenDisplay(NULL);

    xcb_internal_contex.connection =
        XGetXCBConnection(xcb_internal_contex.display);
                               ...
                               uint32 pos_x, uint32 pos_y)
{
    if (m_id == GUI_DEACTIVATED)
    {
        ...
        gui_ptr->platform.display = XOpenDisplay(NULL);

        gui_ptr->platform.connection =
            XGetXCBConnection(gui_ptr->platform.display);
        ...
        xcb_create_window(gui_ptr->platform.connection, XCB_COPY_FROM_PARENT,
                          gui_ptr->platform.window, xcb_internal_contex.window,
