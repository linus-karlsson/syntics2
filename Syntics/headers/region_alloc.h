...
#define stack_malloc(num_elements, type)                                       \
    (type*)i_region_malloc(stack_get(), (num_elements) * sizeof(type),            \
                          _Alignof(type))

#define stack_array(capacity, type)                                            \
    (type*)i_region_array(stack_get(), capacity, sizeof(type), _Alignof(type))

#define stack_calloc(num_elements, type)                                       \
    (type*)i_region_calloc(stack_get(), (num_elements) * sizeof(type),            \
                          _Alignof(type))

#define stack_array0(capacity, type)                                           \
    (type*)i_region_array_calloc(stack_get(), capacity, sizeof(type),           \
                                _Alignof(type))

#define stack_pop_malloc(num_elements, type)                                   \
    i_region_pop(stack_get(), (num_elements) * sizeof(type))

#define stack_pop_array(num_elements, type)                                    \
    i_region_pop(stack_get(), (num_elements) * sizeof(type))

#define region_malloc(region, num_elements, type)                              \
    (type*)i_region_malloc(region, (u32)((num_elements) * sizeof(type)),          \
                          _Alignof(type))

#define region_malloc_struct(region, type)                                     \
    (type*)i_region_malloc(region, (u32)(sizeof(type)), _Alignof(type))

#define region_calloc(region, num_elements, type)                              \
    (type*)i_region_calloc(region, (u32)((num_elements) * sizeof(type)),          \
                          _Alignof(type))

#define region_calloc_struct(region, type)                                     \
    (type*)i_region_calloc(region, (u32)sizeof(type), _Alignof(type))

#define region_array(region, capacity, type)                                   \
    (type*)i_region_array(region, capacity, (u32)sizeof(type), _Alignof(type))

#define region_array_calloc(region, capacity, type)                            \
    (type*)i_region_array_calloc(region, capacity, (u32)sizeof(type),           \
                                _Alignof(type))

#define region_array_copy(region, values, capacity, type)                      \
    (type*)i_region_array_val(region, capacity, (u32)sizeof(type),              \
                             _Alignof(type), values);

#define region_pop(region, num_elements, type)                                 \
    i_region_pop(region, (num_elements) * sizeof(type))

#define region_array_head(array) i_array_check(array)
...
#define region_array_clear(array, type) i_array_clear(array, sizeof(type))
...
#define region_array_pop(array) (array)[i_array_check_pop_size((array))]

#define region_array_value_ptr(array, index)                                   \
    ((array) + i_array_check_size_index((array), (index)))
...
#define stack_begin_scope(stack_name) u64 stack_name = i_stack_begin_scope()
#define stack_end_scope(stack_name) i_stack_end_scope(stack_name);
#define stack_begin_defer_end_scope()                                          \
    u64 THIS_IS_NOT28930167 = i_stack_begin_scope();                            \
    defer({ stack_end_scope(THIS_IS_NOT28930167); })

#define stack_get() i_stack_get(0)
...
Region_Alloc* i_stack_get(u32 check_val);
u64 stack_size(void);
void stack_reset(void);
u64 i_stack_begin_scope(void);
void i_stack_end_scope(u64 size_at_start);
...
void* i_region_array(Region_Alloc* region, u32 capacity, u32 type,
                    u32 alignment);
...
Array_Head* i_array_check(void* array);
b8 i_array_check_size(void* array);
u32 i_array_check_size_index(void* array, u32 index);
u32 i_array_check_pop_size(void* array);
void i_array_clear(void* array, u32 stride);
...
void i_region_pop(Region_Alloc* region, u32 size, Allocation_Type alloc_type);
