...
Mutex syntics_platform_mutex_create(void);
...
Semaphore 
      syntics_platform_semaphore_create(i32 initial_count, i32 max_count);
...
Thread_Handle 
      syntics_platform_thread_create(void* data, thread_return_value (*thread_function)(void* data), unsigned long creation_flag, unsigned long* thread_id);
void  syntics_platform_thread_join(Thread_Handle handle);
void  syntics_platform_thread_destroy(Thread_Handle handle);

u32   syntics_platform_get_core_count(void);
...
void  syntics_platform_event_set_on_key_pressed(Platform* platform, On_Key_Pressed_Callback c);
void  syntics_platform_event_set_on_key_released(Platform* platform, On_Key_Released_Callback c);
void  syntics_platform_event_set_on_button_pressed(Platform* platform, On_Button_Pressed_Callback c);
void  syntics_platform_event_set_on_button_released(Platform* platform, On_Button_Released_Callback c);
void  syntics_platform_event_set_on_mouse_move(Platform* platform, On_Mouse_Moved_Callback c);
void  syntics_platform_event_set_on_mouse_wheel(Platform* platform, On_Mouse_Wheel_Callback c);
void  syntics_platform_event_set_on_window_focused(Platform* platform, On_Window_Focused_Callback c);
void  syntics_platform_event_set_on_window_resize(Platform* platform, On_Window_Resize_Callback c);
void  syntics_platform_event_set_on_window_enter_leave(Platform* platform, On_Window_Enter_Leave_Callback c);
void  syntics_platform_event_set_on_key_stroke(Platform* platform, On_Key_Stroke_Callback c);
